    ...
    for (let x = 0; x < width; x += 1) {
      ...
      for (let ky = -radius; ky <= radius; ky += 1) {
        for (let kx = -radius; kx <= radius; kx += 1) {
          const sampleX = Math.min(width - 1, Math.max(0, x + kx));
          const sampleY = Math.min(height - 1, Math.max(0, y + ky));
          window.push(data[(sampleY * width + sampleX) * 4]);
        }
      }

      window.sort((a, b) => a - b);
      const value = window[Math.floor(window.length / 2)];
      const outputIndex = (y * width + x) * 4;
      result[outputIndex] = value;
      result[outputIndex + 1] = value;
      result[outputIndex + 2] = value;
...
export function localContrast(imageData, radius = 8) {
  const { width, height, data } = imageData;
  const output = cloneImageData(imageData);
  const result = output.data;
  const integral = new Float64Array((width + 1) * (height + 1));
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const mean = sum / area;
      const gray = data[(y * width + x) * 4];
      const value = Math.min(255, Math.max(0, gray + (gray - mean) * 0.8));
      const outputIndex = (y * width + x) * 4;
      result[outputIndex] = value;
      result[outputIndex + 1] = value;
      result[outputIndex + 2] = value;
...
export function sobelEdges(imageData) {
  ...
  for (let y = 1; y < height - 1; y += 1) {
    for (let x = 1; x < width - 1; x += 1) {
      ...
      for (let ky = -1; ky <= 1; ky += 1) {
        for (let kx = -1; kx <= 1; kx += 1) {
          const value = data[((y + ky) * width + (x + kx)) * 4];
      ...
      const outputIndex = (y * width + x) * 4;
      result[outputIndex] = magnitude;
      result[outputIndex + 1] = magnitude;
      result[outputIndex + 2] = magnitude;
    }
  }
...
export function niblackThreshold(imageData, windowSize = 25, k = -0.2) {
  const { width, height, data } = imageData;
  const output = cloneImageData(imageData);
  const result = output.data;
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      for (let ky = -half; ky <= half; ky += 1) {
        for (let kx = -half; kx <= half; kx += 1) {
          const sampleX = Math.min(width - 1, Math.max(0, x + kx));
          const sampleY = Math.min(height - 1, Math.max(0, y + ky));
          const gray = data[(sampleY * width + sampleX) * 4];
          sum += gray;
...
export const PIPELINES = {
  ...
  niblackDot: [
    ...
    (imageData) => localContrast(imageData, 10),
    (imageData) => niblackThreshold(imageData, 27, -0.18),
  ],
  edgeDot: [
    (imageData) => toGrayscale(imageData),
    (imageData) => stretchContrast(imageData),
    (imageData) => sobelEdges(imageData),
    (imageData) => otsuThreshold(imageData),
  ],
  claheOtsu: [
    (imageData) => toGrayscale(imageData),
    (imageData) => localContrast(imageData, 12),
    (imageData) => sharpen(imageData),
    (imageData) => otsuThreshold(imageData),
  ],
  medianAdaptive: [
    ...
    (imageData) => medianFilter(imageData, 1),
    (imageData) => stretchContrast(imageData),
    (imageData) => adaptiveThreshold(imageData, 23, 7),
  ],
};

export const FAST_PIPELINES = ['normal', 'sharpenOtsu', 'claheOtsu'];
